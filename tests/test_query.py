...
def test_query_build_less():
    ...
    result = build_sql_query_string(query_str, 1, 50)
    assert result == 'column_name=lte.12&limit=50'
...
def test_query_build_multiple():
    query_str = ['column_name__exact=BIDULE', 'column_name__greater=12', 'column_name__exact=BIDULE']
    result = build_sql_query_string(query_str, 1, 50)
    assert result == 'column_name=eq.BIDULE&column_name=gte.12&column_name=eq.BIDULE&limit=50'
