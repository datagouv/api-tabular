...
async def test_swagger_endpoint(fake_client, _resource_id):
    res = await fake_client.get(f"/api/resources/{_resource_id}/swagger/")
...
async def test_swagger_no_indexes(fake_client, tables_index_rows, params):
...
async def test_swagger_with_indexes(fake_client, tables_index_rows, exceptions_rows, _resource_id):
    detection = json.loads(tables_index_rows[_resource_id]["csv_detective"])
