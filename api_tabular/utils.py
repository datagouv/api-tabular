...
# Global query builder instance for backward compatibility
_query_builder = QueryBuilder()

# Expose constants for backward compatibility
TYPE_POSSIBILITIES = _query_builder.TYPE_POSSIBILITIES
MAP_TYPES = _query_builder.MAP_TYPES
OPERATORS_DESCRIPTIONS = _query_builder.OPERATORS_DESCRIPTIONS
...
def build_sql_query_string(
    ...
    """Build SQL query string (backward compatible wrapper)."""
    return _query_builder.build_sql_query_string(
        request_arg, resource_id, indexes, page_size, offset
    )


def get_column_and_operator(argument: str) -> tuple[str, str]:
    """Get column and operator from argument (backward compatible wrapper)."""
    return _query_builder.get_column_and_operator(argument)


def add_filter(argument: str, value: str, indexes: set | None) -> tuple[str | None, bool]:
    """Add a filter to the query (backward compatible wrapper)."""
    return _query_builder.add_filter(argument, value, indexes)


def add_aggregator(argument: str, indexes: set | None) -> tuple[str, str]:
    """Add an aggregator to the query (backward compatible wrapper)."""
    return _query_builder.add_aggregator(argument, indexes)


def raise_if_not_index(column_name: str, indexes: set | None) -> None:
    """Raise error if column is not in allowed indexes (backward compatible wrapper)."""
    return _query_builder.raise_if_not_index(column_name, indexes)
